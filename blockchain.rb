module Melon
  class Blockchain
    ...
    def save_block(block)
      ...
      Melon::PendingTransaction.delete_by_id(block.transactions.map { |t| t["id"] })
    end
    ...
    def find_higher_blocks_on_the_network
      ...
      Array(@peers).each do |peer|
        ...
        block = Melon::Block.initialize_from_json(self, peer_response)
    ...
    def validate_and_switch_to_fork(peer, target_height)
      ...
      loop do
        new_blocks << Melon::Block.initialize_from_json(self, JSON.parse(HTTParty.get("http://#{peer}/blocks/#{height}").body))
    ...
    def validate(new_block)
      ...
      new_block.transactions.each do |transaction|
        if Melon::WalletTransfer.find(transaction["id"]).nil? == false
